...
def decode(model_output, output_sizes=(13, 13), num_class=80, anchors=None):
    ...
    obj_probs = tf.nn.sigmoid(detection_result[:, :, :, 4], name="obj_probs")  # 置信度，sigmoid函数归一化到0-1
    class_probs = tf.nn.softmax(detection_result[:, :, :, 5:], name="class_probs")  # 网络回归的是'得分',用 softmax 转变成类别概率
    ...
    bboxes = tf.stack([bbox_x - bbox_w / 2, bbox_y - bbox_h / 2,
                       bbox_x + bbox_w / 2, bbox_y + bbox_h / 2], axis=3, name="bboxes")
