...
bboxes.shape: (1, 169, 5, 4)
obj_probs.shape: (1, 169, 5)
class_probs.shape: (1, 169, 5, 80)
Converted 112 variables to const ops.
487 ops in the final graph.
...
WARNING:caffe2.python.workspace:Original python traceback for operator `2` in network `tf2onnx_predict` in exception above (most recent call last):
	run_onnx FAIL [enforce fail at conv_op_impl.h:182] filter.dim32(1) == kernel_h().  Error from operator: 
input: "resnet_v1_50/conv1/Conv2D__2:0" input: "resnet_v1_50/conv1/weights:0" output: "resnet_v1_50/conv1/Conv2D:0" name: "resnet_v1_50/conv1/Conv2D" type: "Conv" arg { name: "kernels" ints: 7 ints: 7 } arg { name: "dilations" ints: 1 ints: 1 } arg { name: "strides" ints: 2 ints: 2 } device_option { device_type: 0 cuda_gpu_id: 0 }
   


/data1/train_disk1/Train_tools/pytorch/build/caffe2/python/onnx/backend.py:730: UserWarning: This version of onnx-caffe2 targets ONNX operator set version 7, but the model we are trying to import uses version 8.  We will try to import it anyway, but if the model uses operators which had BC-breaking changes in the intervening versions, import will fail.
  warnings.warn("This version of onnx-caffe2 targets ONNX operator set version {}, but the model we are trying to import uses version {}.  We will try to import it anyway, but if the model uses operators which had BC-breaking changes in the intervening versions, import will fail.".format(cls._known_opset_version, imp.version))
WARNING: Logging before InitGoogleLogging() is written to STDERR
W0709 15:32:13.889955 70221 init.h:99] Caffe2 GlobalInit should be run before any other API calls.
W0709 15:32:13.891681 70221 init.h:99] Caffe2 GlobalInit should be run before any other API calls.
result[0].shape: (1, 1, 1, 1000)
result.shape: (1, 1000)
	 -----np.argmax(result): [207]
	run_onnx OK
	Results:  
Not equal to tolerance rtol=0.01, atol=0

(mismatch 0.09999999999999432%)
 x: array([[[[-2.693674e+00,  3.681921e-01, -3.229158e+00, -1.318455e+00,
          -6.279123e-01, -1.245404e+00, -1.251413e+00,  3.112260e-01,
          -5.900652e-01, -1.735047e+00, -3.872965e+00, -1.182940e+00,...
 y: array([[[[-2.693675e+00,  3.681917e-01, -3.229157e+00, -1.318456e+00,
          -6.279121e-01, -1.245405e+00, -1.251414e+00,  3.112265e-01,
          -5.900650e-01, -1.735048e+00, -3.872966e+00, -1.182939e+00,...
...
链接: https://pan.baidu.com/s/1okICzfNKwbtjc6igGSnRoQ 密码: r47i
