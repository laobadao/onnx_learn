...
def tensorflow_to_onnx(graph, shape_override,inputs=None, middle_inputs=None):
    ...
    def delete_op(ops, input, middle_input):
        for node in ops:
            for i, tensor in enumerate(node.inputs):
                # print("all input_name:", tensor.name)
                if tensor.name == input and node.name+":0" != middle_input:
                    # print("node:", node)
                    next_input = node.name+":0"
                    print("next_input:", next_input)
                    print("before remove len(ops):", len(ops))
                    ops.remove(node)
                    print("after remove len(ops):", len(ops))
                    delete_op(ops, next_input, middle_input)
        return ops


    if middle_inputs:

        ops = delete_op(ops, inputs, middle_inputs[0])
        ...
        if "CropAndResize:0" in middle_inputs:
            delete_part2 = {"FirstStageFeatureExtractor", "FirstStageBoxPredictor", "image_tensor", "range",
                            ...
                            'strided_slice_4/stack', 'strided_slice_3/stack', 'strided_slice_2/stack',
            ...
            deleted_op2 = ["Reshape", 'Reshape_1', 'Reshape_2', 'Reshape_3', 'Reshape_4', 'Reshape_5', 'Reshape_6',
                           ...
                           "strided_slice_14/stack_1", "strided_slice_14/stack", 'strided_slice/stack_2',
                           'strided_slice/stack_1',
                           'strided_slice/stack', 'strided_slice', 'Reshape/shape', 'Reshape_1/shape', 'stack/0',
                           'stack/1',
                           'stack_3', 'Softmax', 'Slice', 'concat', 'init', 'Reshape_6/shape', 'concat/axis',
                           'concat/values_0',
                           ]
...
def process_tf_graph(tf_graph, continue_on_error=False, verbose=False, target=None,
    ...
    rewriters = [rewrite_transpose, rewrite_flatten, rewrite_random_uniform,
                 ...
                 rewrite_batchnorm, rewrite_batchnorm1, rewrite_dense]
